...
# nRF Sniffer のセットアップは nrfutil の `ble-sniffer` コマンドで自動化する。
...
# 旧来の SERIAL_PORT（tty パス）はシリアル番号指定に置き換わった。
SERIAL_PORT ?=
...
#   独立ターゲット化により CI が本工程だけを実機実行して検証できる。
# ============================================================
...
#   extcap shim を $(WIRESHARK_EXTCAP_DIR) に配置する。手動 zip は不要。
...
#   ソースツリー（SAMPLE_DIR を含む）は本ターゲットが別途取得する。
#
...
#   フラッシュ済みを前提とする読み取り専用検査。書き込みは deploy が行う。
# ============================================================
